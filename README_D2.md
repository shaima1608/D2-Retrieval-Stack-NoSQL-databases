...
## Fixed submission notes

This version fixes the Docker/local configuration split:

- Local runs use `.env.example` values such as `localhost`.
- Docker Compose uses `.env.docker` values such as `mongo`, `qdrant`, and `neo4j`.
- The embedding model, MongoDB client, Qdrant client, and BM25 index are cached to reduce `/search` latency.
- `/health` and `/stats` check MongoDB, Qdrant, and Neo4j separately.

After running `/ingest`, update the metrics table with:
